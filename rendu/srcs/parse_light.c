...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/02/21 23:46:56 by ggaudin-          #+#    #+#             */
/*   Updated: 2018/03/06 14:27:13 by agaspard         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		stock_light2(t_sce *s, int coor_num, int p)
{
	...
	if (coor_num == 6)
		ret = stock_light_colors(s, coor_num, p);
	if (coor_num == 7)
		ret = stock_light_colors(s, coor_num, p);
	if (coor_num == 8)
		ret = stock_light_colors(s, coor_num, p);
...
int			stock_light(t_sce *s, int coor_num, int j, int p)
{
	...
	while (s->tmp[j])
	{
		if ((s->tmp[j] < 48 || s->tmp[j] > 57) && (s->tmp[j]\
					!= '.') && (s->tmp[j] != '-'))
			ft_exit("light position parameters must be numerical");
		j++;
	}
		...
		while (e->map[p][x++] && x <= len && s->coor < 9)
			if (e->map[p][x - 1] == '[' || s->coor > 0)
						...
						&& s->coor < 9)
				{
					...
					{
						s->coor += stock_light(s, s->coor, j, p - P_SCENE);
						check_light(e, s, p, x + 1);
					}
				}
		return (final_check_forms(e, s, p, LIGHT));
	}
	return (1);
}
